...
export function ColorScrollAnimation() {
    ...
    const itemWidth = clamp(stageSize.width / 7.1, 100, 182);
    const itemHeight = itemWidth * 1.67;
    const centerX = stageSize.width / 2;
    const centerY = stageSize.height * 0.5;
    const circleRadius = clamp(stageSize.width * 0.2, 130, 260);
    const lineSpacing = Math.min(itemWidth * 1.22, (stageSize.width * 0.94) / (colorTubes.length - 1));
    const lineWidth = lineSpacing * (colorTubes.length - 1);
    const lineY = stageSize.height * 0.58;

    return (
        <section ref={sectionRef} className="relative h-[1450px] overflow-x-hidden bg-white px-6 py-24 md:px-10 md:py-28">
            ...
            <div className="sticky top-[5vh] mx-auto mt-8 flex h-[760px] max-w-[2600px] items-center justify-center overflow-visible">
                <div ref={stageRef} className="relative h-[720px] w-full max-w-[2500px]">
                    {colorTubes.map((tube, index) => {
                        ...
                        const lineX = centerX - lineWidth / 2 + index * lineSpacing;
                        const x = interpolate(circleX, lineX, easedProgress);
                        const y = interpolate(circleY, lineY, easedProgress);
                        ...
                        return (
                            <button
                                ...
                                style={style}
                            >
                                <span className="relative block h-full w-full transition-transform duration-500 ease-out group-hover:-translate-y-4 group-hover:scale-110 group-focus-visible:-translate-y-4 group-focus-visible:scale-110">
                                    <img
                                        src={`/colors/${tube.slug}_closed.png`}
                                        alt=""
                                        className="absolute inset-0 h-full w-full object-contain drop-shadow-[0_18px_22px_rgba(18,59,109,0.16)] transition-opacity duration-500 ease-out group-hover:opacity-0 group-focus-visible:opacity-0"
                                    />
                                    <img
                                        src={`/colors/${tube.slug}_opened.png`}
                                        alt=""
                                        className="absolute inset-0 h-full w-full object-contain opacity-0 drop-shadow-[0_24px_28px_rgba(18,59,109,0.2)] transition-opacity duration-500 ease-out group-hover:opacity-100 group-focus-visible:opacity-100"
                                    />
                                </span>
                                <span className="pointer-events-none absolute top-[calc(100%+20px)] left-1/2 min-w-max -translate-x-1/2 px-4 py-2 font-['Instrument_Sans'] text-[26px] font-medium tracking-[0.12em] text-[#123b6d] uppercase opacity-0 transition-all duration-300 ease-out group-hover:translate-y-1 group-hover:opacity-100 group-focus-visible:translate-y-1 group-focus-visible:opacity-100">
                                    {tube.name}
                                </span>
                            </button>
                        );
